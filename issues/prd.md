...
**Token Storage**: Both tokens stored in cookies (access: regular cookie, refresh: HTTP-only cookie)
...
- Token storage handled by browser automatically
...
**Cookie settings:**
- Access token: Regular cookie (required for XSS compatibility with React)
- Refresh token: HTTP-only, Secure, SameSite=Lax
...
**Auth Controller** (Manual testing)
...
- **CSRF protection**: Explicitly disabled (cookies handle authentication).
...
Current application.properties contains variables that should be moved to environment variables. This is a separate security issue to address.
